...
function handleOnCheckEmail(ajaxData){
   if (ajaxData.readyState==4 && ajaxData.status==200)    {    
      ...
      if (privateInd=='Y')  {
         document.getElementById('switchVisibilityResult').innerHTML  ="Your profile has been set to private, so no one can see your posts.";
      }   
      else {
         document.getElementById('switchVisibilityResult').innerHTML  ="Your profile has been set to public, so the others can follow you, and view your posts";
      }
   }
}
